...
// Fetch delivered items from the database
$sql_fetch_order_items = "SELECT * FROM orders WHERE userId = '$user_id' AND orderStatus = 'delivered' ORDER BY order_reference_number";
...
<head>
    ...
    <title>My Delivered Orders</title>
    ...
    <section id="page-header" class="about-header">
        <div class="myordersnav">
            <ul id="ordernavbar">
                ...
                <li><a href="myOrders_delivered.php" class="active">Delivered</a></li>
            ...
            <table width="100%">
                ...
                <tbody>
                        ...
                        while ($row = mysqli_fetch_assoc($result_order_items)) {
                            if ($current_order_reference != $row['order_reference_number']) {
                                if ($current_order_reference != "") {
                                    ...
                                    echo "<tr class='$row_class'><td colspan='9'>Total: $total_price</td></tr>";
                                }
                                $current_order_reference = $row['order_reference_number'];
                                $timestamp = strtotime($row['orderDate']);
                                $date = date('Y-m-d', $timestamp);
                                $total_price = 0;  // Reset total price for new order group
                                $toggle_class = !$toggle_class;
                                $row_class = $toggle_class ? "order-batch-1" : "order-batch-2";
                                echo "<tr class='$row_class'><td colspan='9'><strong>Order Reference Number: " . $current_order_reference . "</strong></td></tr>";
                            }
                    ...
                    <?php
                        ...
                        echo "<tr><td colspan='9'>No products delivered are found.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
