        ...
        <form action="../request/cancel_order_request.php" method="post">
            <table width="100%">
                ...
                <tbody>
                    ...
                    $sql_fetch_order_items = "SELECT * FROM orders WHERE userId = '$user_id' ORDER BY order_reference_number";
                    ...
                    if (mysqli_num_rows($result_order_items) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($result_order_items)) {
                            if ($current_order_reference != $row['order_reference_number'] && $row['orderStatus'] == 'Pending') {
                                if ($current_order_reference != "") {
                                    ...
                                    echo "<tr class='$row_class'><td colspan='9' style='padding:20px;'>
                                            <a href='../request/cancel_order_request.php?ref_num=$current_order_reference' class='cancel-button'>" . ($button_display == 'Pending' ? 'Waiting for cancellation approval..' : 'Cancel Order') . "</a>
                                          </td></tr>";
                                }
