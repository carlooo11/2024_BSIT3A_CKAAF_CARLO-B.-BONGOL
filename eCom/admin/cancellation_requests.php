...
<body>
<section id="header">
  <a href="my_shop.php"><img src="../img/logo.png" class="logo" alt=""></a>
...
<section>
  <div class="myordersnav">
    ...
    <li><a href="orders.php">Pending</a></li>
      ...
      <li><a href="delivered_orders.php">Delivered</a></li>
      <li><a href="completed_orders.php">Completed</a></li>
      <li><a class="active" href="cancellation_requests.php">Cancellation Requests</a></li>
    </ul>
  </div>
...
<div class="container">
  <div class="product-display">
    ...
    $query = "SELECT * FROM orders WHERE orderStatus = 'Pending' AND cancel_request = 'Pending' ORDER BY order_reference_number";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($rows = $result->fetch_assoc()) {
            ...
            if ($orderId !== $currentOrderId) {
                ...
                echo '<tr class="order-group ' . $rowClass . '"><td colspan="8">Order Reference Number: ' . '<strong style="color:lightseagreen;">' . $orderId . '</strong>'. '</td><td>';
                echo '<form id="orderForm' . $orderId . '" class="orderForm" action="../request/update_order_status.php" method="post">';
                echo '<input type="hidden" name="orderId" value="' . $orderId . '">';
                echo '<select class="statbtn1" name="orderStatus" id="orderStatus' . $orderId . '">';
                echo '<option value="Pending"' . ($orderStatus == 'Pending' ? ' selected' : '') . '>Pending</option>';
                echo '<option value="Confirm"' . ($orderStatus == 'Confirm' ? ' selected' : '') . '>Confirm</option>';
                echo '<option value="Shipped"' . ($orderStatus == 'Shipped' ? ' selected' : '') . '>Shipped</option>';
                echo '</select>';
                echo '</td>';
                echo '<td>';
                echo '<select class="statbtn" name="cancelStatus" id="cancelStatus' . $orderId . '">';
                echo '<option value="Pending"' . ($rows['cancel_request'] == 'Pending' ? ' selected' : '') . '>Pending</option>';
                echo '<option value="Approved"' . ($rows['cancel_request'] == 'Approved' ? ' selected' : '') . '>Approve</option>';
                echo '<option value="Rejected"' . ($rows['cancel_request'] == 'Rejected' ? ' selected' : '') . '>Reject</option>';
                echo '</select>';
                echo '<input type="hidden" name="orderId" value="' . $orderId .'">';
                echo '<button type="submit" class="save-btn" id="saveBtn' . $orderId . '" onclick="submitForm(' . $orderId . ')">Save</button>';
                echo '</form>';
                echo '</td></tr>';
            } else {
                $rowClass = $isEven ? 'even' : 'odd';
            ...
            echo '<td><a href="userInfo.php">' . $user_id . '</a></td>';
            echo '<td>' . $prodId . '</td>';
            echo '<td>' . $price . '</td>';
            echo '<td>' . $quantity . '</td>';
            ...
            echo '<td>' . $rows['cancel_request'] . '</td>';
            echo '</tr>';
        }
    } else {
        echo '<tr><td colspan="9">No cancellation requests found.</td></tr>';
    }
    ?>
</tbody>
</table>
...
<script>
    function submitForm(orderId) {
        ...
        var saveBtn = document.getElementById('saveBtn' + orderId);
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                if (responseText.includes("successfully")) {
                    // Change button color and text to indicate success
